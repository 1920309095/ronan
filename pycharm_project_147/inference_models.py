...
# 定义一个函数，用于解析一个由数字和范围组成的字符串，返回一个整数列表
...
# 定义一个函数，用于解析一个由两个浮点数组成的字符串，返回一个浮点数元组
...
def get_init_noise(args,model_type,bs=1):
    ...
    elif model_type in ["projected_gan"]:
        device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        label = torch.zeros([1, args.cur_model.c_dim], device=device)
        print(args.seeds,"args.seeds")
        z = torch.from_numpy(np.random.RandomState(args.seeds[0]).randn(1, args.cur_model.z_dim)).to(device).float()
        init_noise=(z,label,args.trunc,args.noise_mode)

    elif model_type in ["biggan"]:
        # Prepare a input
        truncation = 0.4
        print(args.classification,"args.classification")
...
def from_noise_to_image(args,model,noise0,noise1,noise2,noise3,model_type):
    if model_type in ["ddpm_cifar10"]:
        image = model.input2output(noise,num_inference_steps=50)
    elif model_type in ["projected_gan"]:
        image=model(noise0,noise1,truncation_psi=noise2, noise_mode=noise3)
        ...
        noise0 = torch.tensor(noise0).to('cuda:0')
        noise1 = torch.tensor(noise1).to('cuda:0')
        noise2 = torch.tensor([noise2]).to('cuda:0')
        model = model.to('cuda:0')  # 首先将模型移到cuda:0
        model = nn.DataParallel(model)
        image=model(noise0,noise1,noise2)
...
def get_model(model_type,model_path,args):
    if model_type == "ddpm_cifar10":
        ...
        # 加载预训练的BigGAN模型
        model_id = "biggan-deep-128"
        model = BigGAN.from_pretrained(model_id)

        # # 检查是否有多个GPU可用，如果有，则使用DataParallel
