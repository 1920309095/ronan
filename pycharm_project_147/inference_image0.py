...
def get_classifications(args):
    ...
    if args.picture_source =="ADM"or args.picture_source =="stable_diffusion_v_1_5" or args.picture_source =="Midjourney" \
        ...
        folder_path =f"/hy-tmp/GenImage/GenImage-mini/minitrain/{args.picture_source}/"
        ...
        # 从行内容中获取类型
        args.classification = line_content.split(' ',1)[1].split(',',1)[0].strip()
...
def get_image0(args):
    gt_noise = None
    if args.input_selection == "artifact":
        image_dir="/hy-tmp/generated/"
        file=os.listdir(image_dir)
        selected_file=random.choice(file)
        image_dir=os.path.join(image_dir,selected_file)
    ...
    if args.input_selection == "GenImage":
        print("我这跑了")
        picture_name=get_classifications(args)
        image_dir=f"/hy-tmp/GenImage/GenImage-mini/minitrain/{args.picture_source}/"
        image_dir=os.path.join(image_dir,picture_name)
        image0=cv2.imread(image_dir)
        b,g,r = cv2.split(image0)
        image0 = cv2.merge([r, g, b])
        image0=cv2.resize(image0,(128,128),interpolation=cv2.INTER_AREA)
        image0 = image0/255
        image0 = torch.from_numpy(image0).cuda().clamp(0, 1).permute(2,0,1).unsqueeze(0).float()
    ...
    if args.model_type == "ddpm_cifar10":
        ...
        imsize = 32
    elif args.model_type == "biggan":
        imsize = 128
    elif "cm" in args.model_type:
        imsize = 64
    elif args.model_type in ["sd","sd_unet"]:
        imsize = 512
    elif args.model_type == "projected_gan":
        imsize = 256
