...
parser.add_argument("--input_selection", default="GenImage", type=str, help="The path of dev set.")
parser.add_argument("--distance_metric", default="l2", type=str, help="The path of dev set.")
parser.add_argument("--model_type", default="biggan", type=str, help="The path of dev set.")
        ...
        try:
            ...
            for i in range(args.num_iter):
                ...
                if args.mixed_precision:
                    ...
                    image = from_noise_to_image(args,args.cur_model,cur_noise,init_noise[1],init_noise[2],args.model_type)
                    loss = criterion(image0.detach(),image).mean()

                epoch_num_str=""
                if i==1500:
                    epoch_num_str=str(i)
            if not os.path.exists("/imgs/source"):
                # 如果不存在，创建文件路径
                os.makedirs("/imgs/source")
            if not os.path.exists("/imgs/reconstructed"):
                # 如果不存在，创建文件路径
                os.makedirs("/imgs/reconstructed")
            with torch.no_grad():
                save_img_tensor(image,"/imgs/source"+args.input_selection+"_"+args.picture_source+"_"+args.classification+".png")
                save_img_tensor(image0,"/imgs/reconstructed" + args.input_selection + "_" + args.picture_source + "_" + args.classification + ".png")
